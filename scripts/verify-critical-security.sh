...
course_retirement_test_file="${supabase_root}/tests/database/20260906203031_remove_course_ratings_module.test.sql"
...
for pass in 1 2; do
  supabase db reset --local --version 195 --workdir "${repo_root}"
  supabase storage rm --local --experimental --recursive --yes ss:///course-outlines --workdir "${repo_root}"
  supabase migration up --local --workdir "${repo_root}"
done

supabase test db "${test_file}" --local --workdir "${repo_root}"
supabase test db "${course_retirement_test_file}" --local --workdir "${repo_root}"
