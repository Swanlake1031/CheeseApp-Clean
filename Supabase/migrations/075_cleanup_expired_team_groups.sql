-- 075_cleanup_expired_team_groups.sql
-- Automatically delete expired team posts and their linked activity groups.

BEGIN;

DROP FUNCTION IF EXISTS public.cleanup_expired_team_activity_data();

CREATE FUNCTION public.cleanup_expired_team_activity_data()
RETURNS TABLE (
  deleted_group_count INTEGER,
  deleted_post_count INTEGER
)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  FROM (
    ...
    WHERE p.type = 'team'
      AND (
        ...
        OR (t.event_time IS NOT NULL AND t.event_time < NOW())
      )
  ) AS expired;
  ...
  WHERE g.source_type = 'team'
    AND (
      ...
      OR g.source_post_id = ANY(v_expired_team_ids)
      OR NOT EXISTS (
        SELECT 1
        FROM public.team_posts t
        WHERE t.id = g.source_post_id
      )
    );
  ...
  WHERE p.id = ANY(v_expired_team_ids);

  GET DIAGNOSTICS v_deleted_posts = ROW_COUNT;

  RETURN QUERY
  SELECT v_deleted_groups, v_deleted_posts;
END;
$$;

GRANT EXECUTE ON FUNCTION public.cleanup_expired_team_activity_data()
...
BEGIN
  ...
  IF v_is_expired THEN
    ...
    RAISE EXCEPTION 'Expired team posts cannot keep an activity group';
  END IF;
