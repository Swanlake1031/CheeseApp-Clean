-- 052_home_feed_rent_layering_and_profile_geo.sql
-- 1) Persist profile location into posts.geo when post has no explicit coordinates
-- 2) Geo feed rent layering: same-school paid -> same-school -> non-school paid -> others
-- 3) Nearby content prefers high-view posts

BEGIN;

CREATE OR REPLACE FUNCTION public.sync_post_geo_from_profile()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  IF NEW.geo IS NULL THEN
    SELECT p.last_known_geo
    INTO v_profile_geo
    FROM public.profiles p
    WHERE p.id = NEW.user_id;

    IF v_profile_geo IS NOT NULL THEN
      NEW.geo := v_profile_geo;
    END IF;
  END IF;
...
CREATE TRIGGER trg_posts_geo_from_profile
BEFORE INSERT OR UPDATE OF user_id, geo ON public.posts
FOR EACH ROW
EXECUTE FUNCTION public.sync_post_geo_from_profile();

UPDATE public.posts p
SET geo = pr.last_known_geo
FROM public.profiles pr
WHERE p.geo IS NULL
  AND p.user_id = pr.id
  AND pr.last_known_geo IS NOT NULL;
